* foreval.ado version 4.1    11 Nov 2011
{smcl}
...
Before running foreval
   1. Determine the length of the forecast horizon.  Construct a segmentation
      variable, coded 0 for the estimation segment and 1 for the validation
      segment.  Name this variable {bf:segmnt}, to distinguish it from the spelling
      of the spelling of the segmentation variable used for out-of-estimation
      sample evaluation (which we properly spell as segment).
   2. tsappend, add(#) where # = number of periods over which to forecast
   3. generate the forecast with the variable name of forecast
      use the command predict forecast 
   4. If the time series model contains regressors,  each of the regressors
   ...
   5. This may be accomplished with the help of the prepforeval.ado file.
      The user may prepfare for his forecast by invoking prepforeval.ado.
      The syntax for so doing is 
      {cmd:prepforeval} {datasetname} {depvar} {length of forecast horizon}.
      For example, he may on the command line specify:
      {cmd:prepforeval} wpi2b dlwpi 16
      The program will prepare the extend the time variable 12 periods into
      the forecast horizon, so the {cmd:arima} estimation command can be issued.
...
{cmd:do dlwpicmd} will invoke the automated continuation of the application of the
         {cmd:prepforeval wpi2b dlwpi 16} command using evaluation of the forecast.
{cmd:aircmd}  will invoke an automated version of the {it:ex ante} forecast
        evaluation program with graphing of the forecast. aircmd.ado requires
        the transair2.dta dataset and the foreval.ado file.
...
{title: Other measures of forecast accuracy}
   ...
   Symmetric mape version 3.............(smape3)= 100*h^(-1)*sum{over h}[(|forecast-baseline |)/(|forecast|+|baseline|)/2].
